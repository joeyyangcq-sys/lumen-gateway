...
func TestServicePutNormalizesBodyAndDelegates(t *testing.T) {
	...
	if string(store.lastBody) != `{"id":"9","uri":"/demo"}` {
		t.Fatalf("normalized body = %s", store.lastBody)
	}
...
func TestServicePatchMergesJSON(t *testing.T) {
	store := &fakeStore{
		getResult: Envelope{
			Key:   "/apisix/routes/1",
			Value: json.RawMessage(`{"id":"1","uri":"/demo","plugins":{"request-id":{"header_name":"X-Request-Id"},"limit-count":{"count":10}}}`),
		},
	}
	service := New(store)
	...
	if string(store.lastBody) != `{"id":"1","plugins":{"limit-count":{"count":20}},"uri":"/demo-v2"}` {
		t.Fatalf("patched body = %s", store.lastBody)
	}
